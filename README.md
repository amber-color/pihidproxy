...
- USB ケーブル 2 本（推奨）
  - 1 本目: Pi の「PWR IN」ポート → USB 充電器（5V/1A 以上）
  - 2 本目: Pi の「USB」ポート → PC

> **重要: 電源は PWR IN から取ってください**
>
> Pi Zero には microUSB ポートが 2 つあります。
> ```
> 左: [PWR IN] ← USB充電器（5V/1A以上）で給電
> 右: [USB]    ← PC に接続（キーボードとして認識される）
> ```
> `USB` ポートだけで PC と繋ぐと、Pi が起動時に電力不足となり WiFi が接続できません。
> PWR IN から別途給電することで、WiFi と USB HID が同時に動作します。
...
> **前提:** PWR IN に充電器を繋いで Pi を給電すると、USB を PC に繋いでいても WiFi で SSH 接続できます。
