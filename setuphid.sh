...
GADGET=/sys/kernel/config/usb_gadget/piproxy

# If gadget is already configured and bound to UDC, nothing to do
if [ -d "$GADGET" ] && [ -s "$GADGET/UDC" ]; then
    echo "USB gadget already active"
    exit 0
fi

# If gadget directory exists but not bound, just bind it
if [ -d "$GADGET" ]; then
    echo "USB gadget exists, binding to UDC..."
